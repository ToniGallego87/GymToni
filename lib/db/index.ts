...
function insertExercise(runner: SQLiteRunner, row: ExerciseRow): Promise<unknown> {
  return runner.runAsync(
    ...
    [row.id, row.workout_days_id, row.name, row.exercise_order, row.target_reps, row.target_sets]
  );
}

function insertWorkoutLog(runner: SQLiteRunner, row: WorkoutLogRow): Promise<unknown> {
  return runner.runAsync(
    'INSERT INTO workout_logs (id, routines_id, workout_days_id, date, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
    [row.id, row.routines_id, row.workout_days_id, row.date, row.created_at, row.updated_at]
  );
}

function insertExerciseLog(runner: SQLiteRunner, row: ExerciseLogRow): Promise<unknown> {
  return runner.runAsync(
    'INSERT INTO exercise_logs (id, workout_logs_id, exercises_id, exercise_name, exercise_order, raw_input, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    [row.id, row.workout_logs_id, row.exercises_id, row.exercise_name, row.exercise_order, row.raw_input, row.notes, row.created_at]
  );
}
...
function insertCardioLog(runner: SQLiteRunner, row: CardioLogRow): Promise<unknown> {
  return runner.runAsync(
    'INSERT INTO cardio_logs (id, workout_logs_id, type, raw_input, duration, distance, pace, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    [row.id, row.workout_logs_id, row.type, row.raw_input, row.duration, row.distance, row.pace, row.notes]
  );
}

async function insertLogRows(runner: SQLiteRunner, log: WorkoutLog): Promise<void> {
...
export async function loadAppDataFromDb(): Promise<WorkoutAppData | null> {
  ...
  const settings = await db.getAllAsync<SettingRow>('SELECT key, value FROM settings');
  ...
  const rows: DbRows = {
    ...
    workoutDays: await db.getAllAsync<WorkoutDayRow>('SELECT * FROM workout_days'),
    exercises: await db.getAllAsync<ExerciseRow>('SELECT * FROM exercises'),
    workoutLogs: await db.getAllAsync<WorkoutLogRow>('SELECT * FROM workout_logs'),
    exerciseLogs: await db.getAllAsync<ExerciseLogRow>('SELECT * FROM exercise_logs'),
...
async function upsertDayWithExercises(
  ...
  await runner.runAsync(
    `INSERT INTO workout_days (id, routines_id, day_number, name, emoji, description)
     ...
     ON CONFLICT(id) DO UPDATE SET
       ...
       description = excluded.description`,
    [dayRow.id, dayRow.routines_id, dayRow.day_number, dayRow.name, dayRow.emoji, dayRow.description]
  );
  ...
  await runner.runAsync('DELETE FROM exercises WHERE workout_days_id = ?', [day.id]);
...
export async function dbUpdateDay(routineId: string, day: WorkoutDay): Promise<void> {
  const db = await getDb();
  await db.withExclusiveTransactionAsync(async txn => {
...
export async function dbSetActiveRoutine(routineId: string | undefined): Promise<void> {
  const db = await getDb();
  if (routineId) {
    await db.runAsync(
      `INSERT INTO settings (key, value) VALUES (?, ?)
    ...
    await db.runAsync('DELETE FROM settings WHERE key = ?', [SETTING_ACTIVE_ROUTINE_ID]);
  }
}
