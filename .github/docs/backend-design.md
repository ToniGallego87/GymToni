...
- **Fase 4 — Social — HECHO ✅** (SQL ejecutado en Supabase; verificado en
  dispositivo). Esquema de la nube (`supabase/social-schema.sql`:
  `routines.is_public`, tablas `follows` y `routine_likes`, RLS de lectura pública
  para perfiles/rutinas/plan, y la función `popular_routines`) + capa de datos
  (`lib/cloud/social.ts`). UI: **Comunidad** como pestaña de la barra
  (`features/workout/CommunityScreen.tsx`) con tablón de populares (like + clonar),
  **buscar usuarios**, pestañas Populares/**Siguiendo** (feed) y aviso de nuevos
  seguidores; **publicar rutina** (interruptor en `RoutineDetailScreen`); **perfil
  público** editable con foto/avatar (base64) y contadores en `CloudScreen`; ver
  perfil de otro y **seguir** (`UserProfileScreen`) y listas de seguidos/seguidores
  (`FollowingScreen`). Ver §7. **Cabo suelto:** avatares en base64 (migrar a Storage,
  ver ROADMAP).
